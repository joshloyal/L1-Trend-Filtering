 ...
 *                                                                          *
 ****************************************************************************/
int l1tf(const int n, const double *y, const double lambda, double *x, int verbose)
{
    /* parameters */
    ...
    {
        if (verbose) { fprintf(stderr,"Changing to LU factorization\n"); }
        ddtf_chol = FALSE;
        dptr = DDTF;
        for (i = 0; i < m; i++)
        {
    ...
    for (iters = 0; iters <= MAXITER; iters++)
    {
        ...
        if (verbose) { fprintf(stderr,"%6d %15.4e %13.5e %10.2e\n",iters,pobj,dobj,gap); }

        /* STOPPING CRITERION */

        if (gap <= TOL)
        {
            if (verbose) { fprintf(stderr,"Solved\n"); }
...
double l1tf_lambdamax(const int n, double *y, int verbose)
{
    int i, m, info;
    double maxval;
    double *vec, *mat, *dptr;
